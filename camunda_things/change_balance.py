...
def change_balance_service(url, worker_id):
    variables = ['receiver_acc', 'sender_acc', 'amount_to_send']
    print("Started changer")
    while True:
        # try:
        fetch_and_lock = pycamunda.externaltask.FetchAndLock(url=url, worker_id=worker_id, max_tasks=10)
        fetch_and_lock.add_topic(name='change_balance_process', lock_duration=10000, variables=variables)
        # tasks = fetch_and_lock()

        tasks = fetch_and_lock()
        if len(tasks) > 0:
            print("change_balance_service tasks", tasks)
        ...
        for task in tasks:
            ...
            response = requests.get("http://127.0.0.1:8004/change_balance", params=payload)
